...
#include "../include/ccompra/lz4.h"

CCOMPRA_PUBLIC_FUNC
char* lz4_compress(const char *input) {
    ...
    while (pos < inLen) {
        int bestMatchLen = 0;
        int bestOffset = 0;
        size_t start = (pos >= LZ4_WINDOW) ? pos - LZ4_WINDOW : 0;
...
CCOMPRA_PUBLIC_FUNC
char* lz4_decompress(const char *compressed) {
    size_t outCap = 256;
    char *output = malloc(outCap);
    size_t outLen = 0;
    output[0] = '\0';
