...
CCOMPRA_PUBLIC_FUNC
DeflateCompressed* deflate_compress(const char *input) {
    char *lz77Tokens = lz77_compress(input);
    DeflateCompressed *huffOut = huffman_compress(lz77Tokens);
    free(lz77Tokens);
    return huffOut;
}

CCOMPRA_PUBLIC_FUNC
char* deflate_decompress(DeflateCompressed *compressed) {
