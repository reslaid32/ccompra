...
#include "../include/ccompra/lz5.h"

CCOMPRA_PUBLIC_FUNC
char* lz5_compress(const char *input) {
    ...
    while (pos < inLen) {
        int bestMatchLen = 0;
        int bestOffset = 0;
        size_t start = (pos >= LZ5_WINDOW) ? pos - LZ5_WINDOW : 0;
...
CCOMPRA_PUBLIC_FUNC
char* lz5_decompress(const char *compressed) {
    size_t outCap = 256;
    char *output = malloc(outCap);
    if (!output) { perror("malloc"); exit(EXIT_FAILURE); }
    size_t outLen = 0;
